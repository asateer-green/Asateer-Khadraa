...
export type PortfolioForm = typeof EMPTY_PORTFOLIO_FORM & { galleryItems: GalleryItemState[] };
...
export function PortfolioModal({
  ...
  const [form, setForm] = useState<typeof EMPTY_PORTFOLIO_FORM>(EMPTY_PORTFOLIO_FORM);
  ...
  const set = <K extends keyof typeof EMPTY_PORTFOLIO_FORM>(key: K, value: typeof EMPTY_PORTFOLIO_FORM[K]) =>
      ...
      <form id="portfolio-form" onSubmit={handleSubmit} className="p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label={ar ? "العنوان بالعربي" : "Title (Arabic)"} required>
            <TextInput value={form.title_ar} onChange={e => set("title_ar", e.target.value)} required dir="rtl" placeholder="اسم العمل" title="title_ar" />
          </Field>
          <Field label={ar ? "العنوان بالإنجليزي" : "Title (English)"} required>
            <TextInput value={form.title_en} onChange={e => set("title_en", e.target.value)} required dir="ltr" placeholder="Project name" title="title_en" />
          </Field>
        </div>
        ...
        <Field label={ar ? "التصنيف" : "Category"}>
          ...
          >
            ...
            {categories.map(cat => (
              <option key={cat.id} value={cat.id}>{ar ? cat.name_ar : cat.name_en}</option>
            ))}
          </select>
        </Field>
        ...
        <Switch checked={form.is_featured} onChange={v => set("is_featured", v)} label={ar ? "عمل مميز" : "Featured Item"} description={ar ? "يظهر في الرئيسية" : "Shown on homepage"} />

        {/* الـ Gallery المطور المشترك (رفع + روابط) */}
