...
const mockServices: ServiceItem[] = [
  {
    ...
    image: mediaImg,
    icon: <Palette className="w-5 h-5 text-emerald-950 dark:text-emerald-900" />,
  },
  {
    id: "manufacturing",
    ...
    icon: <Layers className="w-5 h-5 text-emerald-950 dark:text-emerald-900" />,
  },
  {
    id: "printing",
    ...
    image: printImg,
    icon: <Printer className="w-5 h-5 text-emerald-950 dark:text-emerald-900" />,
  },
  {
    id: "gifts",
    ...
    image: giftsImg,
    icon: <Gift className="w-5 h-5 text-emerald-950 dark:text-emerald-900" />,
  },
];
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {mockServices.map((svc) => {
            ...
            return (
              <div
                key={svc.id}
                className="group relative aspect-square w-full rounded-2xl overflow-hidden border border-(--color-border) bg-zinc-950 transition-smooth hover:-translate-y-1.5 hover:border-(--color-brand) shadow-card"
              >
                <div className="absolute inset-0 z-0 overflow-hidden bg-(--color-bg-primary)/10 dark:bg-(--color-bg-primary)/25">
                  ...
                  />
                  <div className="absolute inset-0 bg-linear-to-t from-(--color-surface) via-(--color-surface)/40 to-transparent" />
                </div>

                <div className="absolute top-5 right-5 rtl:left-5 rtl:right-auto z-20 w-9 h-9 rounded-xl bg-(--color-brand) flex items-center justify-center shadow-glow">
                  {svc.icon}
                </div>

                <div className="absolute inset-x-0 bottom-0 z-10 p-5 md:p-6 flex flex-col items-start text-start">
                  <h3 className="text-lg font-bold text-(--color-foreground) mb-2 group-hover:text-(--color-brand) transition-colors line-clamp-1">
                    {title}
                  </h3>
                  <p className="text-xs text-(--color-muted-foreground) leading-relaxed font-normal line-clamp-3 group-hover:text-(--color-foreground) transition-colors">
                    {description}
                  </p>
                </div>
              </div>
