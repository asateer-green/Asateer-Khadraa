...
export const GiftsServicePage: React.FC = () => {
  const { t, language } = useLanguageContext();

  const isRtl = language === "ar";
  const ArrowIcon = isRtl ? ArrowLeft : ArrowRight;

  // جلب النقاط الفرعية الخاصة بقسم الهدايا الدعائية
  const bullets: string[] = [
    t("services.items.gifts.bullets.0"),
    t("services.items.gifts.bullets.1"),
    t("services.items.gifts.bullets.2"),
    t("services.items.gifts.bullets.3"),
  ];
        ...
        <div className="p-8 rounded-3xl bg-(--color-surface)/90 backdrop-blur-md border border-(--color-brand)/20 shadow-glow space-y-6">
          ...
          <div className="pt-4 border-t border-(--color-border) flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2.5 px-7 py-3.5 rounded-full bg-(--color-brand) text-emerald-950 dark:text-zinc-950 font-bold text-sm shadow-md hover:scale-[1.03] transition-transform duration-200"
            >
...
export default GiftsServicePage;