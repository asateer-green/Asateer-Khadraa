...
export const PackagingServicePage: React.FC = () => {
  const { t, language } = useLanguageContext();

  const isRtl = language === "ar";
  const ArrowIcon = isRtl ? ArrowLeft : ArrowRight;

  // جلب النقاط الفرعية الخاصة بقسم التغليف والعلب
  const bullets: string[] = [
    t("services.items.packaging.bullets.0"),
    t("services.items.packaging.bullets.1"),
    t("services.items.packaging.bullets.2"),
    t("services.items.packaging.bullets.3"),
  ];
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <nav className="flex items-center justify-center gap-2 text-xs font-bold text-(--color-muted-foreground) uppercase tracking-wider">
          ...
          <span className="text-(--color-brand) font-extrabold">
            {t("services.items.packaging.title")}
          </span>
        </nav>
        ...
        <div className="text-center space-y-4">
          ...
          <p className="max-w-2xl mx-auto text-base sm:text-lg text-(--color-muted-foreground) leading-relaxed">
            {t("services.items.packaging.desc")}
          </p>
        </div>
